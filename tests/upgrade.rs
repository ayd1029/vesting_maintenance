...
pub mod vesting {                                      // Start of the Anchor program module
    ...
    pub fn do_vesting(
        ...
        let cal_amount = vesting_plan
            .plans
            .iter_mut()
            .filter(|p| !p.released)
            .map(|unreleased_p| unreleased_p.amount)
            .sum();
...
pub struct DoVesting<'info> {                       // do_vesting context definition
    ...
    // --- Todo: change address to yours ---
    #[account(
        mut,
        address = "4P33Nb5mrjidVUbLTvo7eNCZeCfyteRu4DaHgDCQfa7z".parse::<Pubkey>().unwrap() @ VestingError::InvalidMint
    )]
